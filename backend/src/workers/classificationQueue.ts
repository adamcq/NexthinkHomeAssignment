...
export const classificationQueue = new Bull('article-classification', config.redis.url, {
    // redis: { // Optional, enable only if admin needed
...
// create a method which will be called to process jobs - it only requires articleId as the argument
...
classificationQueue.process(async (job) => {
    ...
    return await classifyArticle(articleId);
});
